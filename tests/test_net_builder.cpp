...
BOOST_AUTO_TEST_CASE(test_net_builder)
{
    ...
    error = EN_init(ph, (char *)"net.rpt", (char *)"net.out", EN_GPM, EN_HW);
    ...
    error = EN_saveinpfile(ph, (char *)"net_builder.inp");
    BOOST_REQUIRE(error == 0);
    ...
    EN_createproject(&ph);
    error = EN_open(ph, (char *)"net_builder.inp", path_rpt.c_str(), path_out.c_str());
    BOOST_REQUIRE(error == 0);
