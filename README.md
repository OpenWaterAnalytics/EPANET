...
## Getting Started
Fork/Clone the repo. To build the library/CLI and run tests locally:

```
mkdir buildproducts && cd buildproducts
cmake .. -DBUILD_TESTS=1 && make
cd ../tests/regression
pipenv install
pipenv run python3 test_local.py
```

The above command sequence will build EPANET, and then download a suite of test networks along with benchmark results for each one. The tests will fail if hydrualic results are outside predefined numerical tolerances.
