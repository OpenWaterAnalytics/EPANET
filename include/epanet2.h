...
typedef enum { EN_ITERATIONS = 0, EN_RELATIVEERROR = 1 } EN_AnalysisStatistic;
...
typedef enum { EN_JUNCTION = 0, EN_RESERVOIR = 1, EN_TANK = 2 } EN_NodeType;
 ...
 */
int DLLEXPORT ENepanet(char *inpFile, char *rptFile, char *binOutFile,
                       void (*callback)(char *));
...
/**
 ...
 @return error code
 */
int DLLEXPORT ENinit(char *rptFile, char *binOutFile, int UnitsType,
                     int HeadlossFormula);
...
/**
 ...
 @param initFlag 2-digit flag where 1st (left) digit indicates if link flows
 should be re-initialized (1) or not (0), and 2nd digit indicates if hydraulic
...
/**
 ...
 @param[out] tStep Time (seconds) until next hydraulic event. 0 marks end of
 simulation period.
 @return Error code

 This function is used in a loop with ENrunH() to run an extended period
...
/**
 ...
 @param[out] tStep Time in seconds until next hydraulic event. 0 marks end of
 simulation period.
 @return Error code
...
/**
 ...
 @param controlIndex Control index (position of control statement in the input
 ...
 @param[out] nodeIndex Index of controlling node (0 for TIMER or TIMEOFDAY
 control)
 @param[out] level Control level (tank level, junction pressure, or time
 (seconds))
 @return Error code
 */
int DLLEXPORT ENgetcontrol(int controlIndex, int *controlType, int *linkIndex,
                           EN_API_FLOAT_TYPE *setting, int *nodeIndex,
...
/**
 ...
 @param maxLen The maximum number of characters to copy into the char pointer
 errmsg
 @return Error code
 */
...
/**
 ...
 @param[out] id The string ID of the specified node. Up to MAXID characters will
 be copied, so id must be pre-allocated by the calling code to hold at least
...
/**
 @brief Get the index of the demand pattern assigned to a node for a category
 index.
 @param nodeIndex The index of a node (first node is index 1).
 ...
 @return Error code
 */
int DLLEXPORT ENgetdemandpattern(int nodeIndex, int demandIndex,
                                 int *pattIndex);
...
/**
 ...
 @param[out] id The ID of the link. Up to MAXID characters will be copied, so id
 must be pre-allocated by the calling code to hold at least that many
 characters.
 @return Error code
 @see ENgetlinkindex
 */
...
/**
 ...
 @param[out] id The curve's string ID. Client code must preallocate at least
 MAXID characters.
...
/**
 ...
 @param[out] outType The integer-typed pump curve type signifier (output
 parameter)
 @return Error code
 @see EN_CurveType
 */
...
/**
 @brief Get the version number. This number is to be interpreted with implied
 decimals, i.e., "20100" == "2(.)01(.)00"
...
/**
 ...
 @param nindex The index of a node used to control the link, or 0 for TIMER /
 TIMEOFDAY control.
...
/**
 ...
 chemname and chemunits only apply when WQ analysis is for chemical. tracenode
 only applies when WQ analysis is source tracing.
 */
int DLLEXPORT ENsetqualtype(int qualcode, char *chemname, char *chemunits,
                            char *tracenode);

/**
 @brief Get quality analysis information (type, chemical name, units, trace node
 ID)
...
/**
 @brief Gets the number of premises, true actions, and false actions and the
 priority of an existing rule-based control.
 @param index The index of a rule-based control.
 @param nPremises The number of conditions in a rule-based control.
 @param nTrueActions The number of actions that are executed when the conditions
 in the rule-based control are met.
 @param nFalseActions The number of actions that are executed when the
 conditions in the rule-based control are not met.
...
/**
 @brief Gets the components of a premise/condition in an existing rule-based
 control.
 ...
 @return Error code.
 */
int DLLEXPORT ENgetpremise(int indexRule, int indexPremise, int *logop,
                           int *object, int *indexObj, int *variable,
                           int *relop, int *status, EN_API_FLOAT_TYPE *value);

/**
 @brief Sets the components of a premise/condition in an existing rule-based
 control.
 @param indexRule The index of a rule-based control.
 @param indexPremise The index of the premise.
 @param logop The logiv operator (IF/AND/OR) in the premise
...
/**
 @brief Sets the index of an object in a premise of an existing rule-based
 control.
 @param indexRule The index of a rule-based control.
 @param indexPremise The index of the premise.
 @param indexObj The index of the object (e.g. the index of the tank).
...
/**
 ...
 @param indexRule The index of a rule-based control.
 @param indexAction The index of the action when the conditions in the rule are
 met.
...
int DLLEXPORT EN_open(EN_Project *p, char *inpFile, char *rptFile,
                      char *binOutFile);
...
int DLLEXPORT EN_getcontrol(EN_Project *p, int controlIndex, int *controlType,
...
int DLLEXPORT EN_getoption(EN_Project *p, EN_Option opt,
                           EN_API_FLOAT_TYPE *value);
...
int DLLEXPORT EN_getstatistic(EN_Project *p, int code,
                              EN_API_FLOAT_TYPE *value);
...
int DLLEXPORT EN_getcurve(EN_Project *p, int curveIndex, char *id, int *nValues,
                          EN_API_FLOAT_TYPE **xValues,
                          EN_API_FLOAT_TYPE **yValues);
int DLLEXPORT EN_getheadcurveindex(EN_Project *p, int pumpIndex,
                                   int *curveIndex);
int DLLEXPORT EN_setheadcurveindex(EN_Project *p, int pumpIndex,
                                   int curveIndex);
...
int DLLEXPORT EN_setpattern(EN_Project *p, int index, EN_API_FLOAT_TYPE *f,
                            int len);
int DLLEXPORT EN_setpatternvalue(EN_Project *p, int index, int period,
                                 EN_API_FLOAT_TYPE value);
...
int DLLEXPORT EN_getrule(EN_Project *p, int index, int *nPremises,
                         int *nTrueActions, int *nFalseActions,
                         EN_API_FLOAT_TYPE *priority);
int DLLEXPORT EN_setrulepriority(EN_Project *p, int index,
                                 EN_API_FLOAT_TYPE priority);
int DLLEXPORT EN_getpremise(EN_Project *p, int indexRule, int indexPremise,
                            int *logop, int *object, int *indexObj,
                            int *variable, int *relop, int *status,
                            EN_API_FLOAT_TYPE *value);
int DLLEXPORT EN_setpremise(EN_Project *p, int indexRule, int indexPremise,
                            int logop, int object, int indexObj, int variable,
                            int relop, int status, EN_API_FLOAT_TYPE value);
int DLLEXPORT EN_setpremiseindex(EN_Project *p, int indexRule, int indexPremise,
                                 int indexObj);
int DLLEXPORT EN_setpremisestatus(EN_Project *p, int indexRule,
                                  int indexPremise, int status);
int DLLEXPORT EN_setpremisevalue(EN_Project *p, int indexRule, int indexPremise,
                                 EN_API_FLOAT_TYPE value);
int DLLEXPORT EN_gettrueaction(EN_Project *p, int indexRule, int indexAction,
                               int *indexLink, int *status,
                               EN_API_FLOAT_TYPE *setting);
int DLLEXPORT EN_settrueaction(EN_Project *p, int indexRule, int indexAction,
                               int indexLink, int status,
                               EN_API_FLOAT_TYPE setting);
int DLLEXPORT EN_getfalseaction(EN_Project *p, int indexRule, int indexAction,
                                int *indexLink, int *status,
                                EN_API_FLOAT_TYPE *setting);
int DLLEXPORT EN_setfalseaction(EN_Project *p, int indexRule, int indexAction,
                                int indexLink, int status,
                                EN_API_FLOAT_TYPE setting);
