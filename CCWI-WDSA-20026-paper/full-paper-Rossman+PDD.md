...
The ability to model pressure dependent demands was added to EPANET in version 2.2. It used the Wagner bounded power law to express nodal demand `D` as a function of head `H` as follows:

```
D = Dreq * [(H - Hmin) / (Hreq - Hmin)]^(1/C)
```

where `Dreq` is the node's required demand, `Hreq` is the head needed to supply the required demand, `Hmin` is a head below which demand is zero, and `(1/C)` is an exponent usually set to 0.5 to mimic flow through an orifice. `D` is constrained to be 0 when `H <= Hmin` and to equal `Dreq` when `H >= Hreq`.

EPANET 2.2 used the inverted form of the demand function to treat pressure-dependent demands in an analogous fashion to how it models flow through emitter elements. While emitter flows are unconstrained, demand flows are forced to be between 0 and full required demand. As with emitters, the demand flow out of a node at head `H` is considered as flow through a virtual pipe connected to a virtual reservoir node with fixed grade. The head loss `Hloss` across the virtual pipe is computed using the inverse of the demand function:

```
Hloss = (Hreq - Hmin) * (D/Dreq)^C   for `0 < D < Dreq
```

For `D <= 0`, `Hloss = BIG * D` while for `D >= Dreq`, `Hloss = BIG * (D - Dreq)`. `BIG` is a large positive number, such as 10^9, that constrains demand to be between 0 and Dreq. This expression for head loss as a function of demand was incorporated into the EPANET hydraulic solver in the same manner as emitters, without affecting the size or structure of the solution matrix used by the solver.

The linear barrier function used in the inverted demand equation by EPANET 2.2 has non-continuous derivatives that vanish as demand flow becomes zero. These attributes can negatively impact the convergence of the Newton-Raphson scheme used by the hydraulic solver. To improve on this, EPANET 2.3 introduced an alternative nonlinear barrier function into the virtual pipe's head loss function to limit demands to be between 0 and `Dreq`:

```
Hloss = (Hreq - Hmin) * (D/Dreq)^C + (Al - Bl)/2 + (Au - Bu)/2
```

where `Al = BIG * D`, `Bl = (Al^2 + e)^0.5`, `Au = BIG * (D - Dreq)`, `Bu = (Au^2 + e)^0.5` and `BIG` is again a large positive number while `e` is a small positive number, such as 10^-6. This function is smooth with continuous first derivatives that are always non-zero. Over the range of 0 to `Dreq`, the function faithfully follows the original inverted demand function. Outside this range, the barrier terms quickly increase the head loss across the virtual pipe forcing demands to stay within the required range.

Todini et al. (2022) compared the convergence performance of the linear barrier function used in EPANET 2.2 with that of version 2.3's new nonlinear barrier function. Out of 42 test runs on 14 different networks, version 2.3 showed significant convergence improvement in seven of these without affecting convergence performance in the rest. EPANET 2.3 has also applied the lower sided nonlinear barrier function to emitters that users configure to not allow backflow, further enhancing EPANET's flexibility to represent real-world conditions.
...
Shang, F.,  H. Woo,  J. B. Burkhardt, R. Murray (2021). “Lagrangian Method to Model Advection-Dispersion-Reaction Transport in Drinking Water Pipe Networks”, Journal of Water Resources Planning and Management, 147(9): 04021057. (doi: 10.1061/(ASCE)WR.1943-5452.0001421).
...
Todini, E., Farina, A., Gabriele, A., Gargano, R., and Rossman, L.A. (2022) "Comparing alternative PDA solvers with EPANET", J. of Hydroinformatics, 24(3):697. (doi: 10.2166/hydro.2022.005).
