...
int EXPORT_PY_API rpt_copy(Handle ph, char *filename)
{
    handle_t *pr = (handle_t *)ph;
    return error_set(pr->error, EN_copyreport(pr->project, filename));
}
...
{
    handle_t *pr = (handle_t *)ph;
    return error_set(pr->error, EN_addnode(pr->project, id, nodeType, index));
}
...
int EXPORT_PY_API node_setjuncdata(Handle ph, int index, double elev, double dmnd, char *dmndpat)
{
    handle_t *pr = (handle_t *)ph;
    return error_set(pr->error, EN_setjuncdata(pr->project, index, elev, dmnd, dmndpat);
}    

int  EXPORT_PY_API node_settankdata(Handle ph, int index, double elev, double initlvl,
                   double minlvl, double maxlvl, double diam, double minvol, char *volcurve)
{
     handle_t *pr = (handle_t *)ph;
     return error_set(pr->error, EN_settankdata(pr->project, index, elev, initlvl, minlvl,
...
int EXPORT_PY_API dmnd_add(Handle ph, int nodeIndex, double baseDemand, int patIndex, char *demandName)
{
    handle_t *pr = (handle_t *)ph;
    return error_set(pr->error, EN_adddemand(pr->project, nodeIndex, baseDemand, patIndex, demandName));
}

int EXPORT_PY_API dmnd_delete(Handle ph, int nodeIndex, int demandIndex)
{
    handle_t *pr = (handle_t *)ph;
    return error_set(pr->error, EN_deletedemand(pr->project, nodeIndex, demandIndex));
}
...
{
    handle_t *pr = (handle_t *)ph;
    return error_set(pr->error, EN_addlink(pr->project, id, linkType, fromNode, toNode, index));
}
...
int EXPORT_PY_API link_setpipedata(Handle ph, int index, double length, double diam,
                                   double rough, double mloss)
{
    handle_t *pr = (handle_t *)ph;
    return error_set(pr->error, EN_setpipedata(pr->project, index, length, diam, rough, mloss));
}                                   
...
int EXPORT_PY_API ptrn_delete(Handle ph, int index)
{
    handle_t *pr = (handle_t *)ph;
    return error_set(pr->error, EN_deletepattern(pr->project, index));
}
...
int EXPORT_PY_API ptrn_setid(Handle ph, int index, char *id)
{
    handle_t *pr = (handle_t *)ph;
    return error_set(pr->error, EN_setpatternid(pr->project, index, id));
}
...
int EXPORT_PY_API curv_delete(Handle ph, int index)
{
    handle_t *pr = (handle_t *)ph;
    return error_set(pr->error, EN_deletecurve(pr->project, index));
}
...
int EXPORT_PY_API curv_setid(Handle ph, int index, char *id);
{
    handle_t *pr = (handle_t *)ph;
    return error_set(pr->error, EN_setcurveid(pr->project, index, id));
}
...
int EXPORT_PY_API rcntl_getinfo(Handle ph, int index, int *nPremises, int *nThenActions, int *nElseActions, double *priority)
{
    handle_t *pr = (handle_t *)ph;
    return error_set(pr->error, EN_getruleinfo(pr->project, index, nPremises, nThenActions, nElseActions, priority));
}
