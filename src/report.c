...
// Local functions
static void writenodetable(EN_Project pr, Pfloat *);
static void writelinktable(EN_Project pr, Pfloat *);
static void writeenergy(EN_Project pr);
static int  writeresults(EN_Project pr);
static int  disconnected(EN_Project pr);
static void marknodes(EN_Project pr, int, int *, char *);
static void getclosedlink(EN_Project pr, int, char *);
static void writelimits(EN_Project pr, int, int);
static int  checklimits(report_options_t *rep, double *, int, int);
static char *fillstr(char *, char, int);
static int  getnodetype(network_t *net, int);
...
*/
{
  ...
  {
    sprintf(s, FMT33, (float)qual->Qstep / 60.0);
    writeline(pr, s);
...
*/
{
  ...
  for (i = 1; i <= net->Ntanks; i++)
  {
    ...
    else newstat = hyd->OldStat[net->Nlinks + i];
...
int checklimits(report_options_t *rpt, double *y, int j1, int j2)
/*
**--------------------------------------------------------------
...
int getnodetype(network_t *net, int i)
/*
**---------------------------------------------------------
