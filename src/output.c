...
// Local functions
static int  nodeoutput(EN_Project pr, int, REAL4 *, double);
static int  linkoutput(EN_Project pr, int, REAL4 *, double);
static int  savetimestat(EN_Project pr, REAL4 *, HdrType);
static int  savenetreacts(EN_Project pr, double, double, double, double);
static int  saveepilog(EN_Project pr);
...
*/
{
    ...
    for (i = 1; i <= net->Nlinks; i++)
    {
        ...
        else x[i] = (REAL4)hyd->LinkFlows[i];
    }
...
*/
{
    ...
    {
        pump = &net->Pump[i];
        if (hdur == 0.0) pump->Energy[TOTAL_COST] *= 24.0;
        else
        {
        ...
        else pump->Energy[KWH_PER_FLOW] *= (1.0e6 / GPMperCFS / 60.);

        // ... save energy stats to REAL4 work array
        for (j = 0; j < MAX_ENERGY_STATS; j++) x[j] = (REAL4)pump->Energy[j];

        // ... save energy results to output file
        index = pump->Link;
        if (fwrite(&index, sizeof(INT4), 1, outFile) < 1) return 308;
        if (fwrite(x, sizeof(REAL4), MAX_ENERGY_STATS, outFile) <
            MAX_ENERGY_STATS) return 308;
    }
...
*/
{
    ...
    *hydtime = t;
    ...
    else for (i = 1; i <= net->Nlinks; i++) hyd->LinkFlows[i] = x[i];
        ...
        for (i = 1; i <= net->Nlinks; i++)
        {
            if (net->Link[i].Type <= PIPE && ABS(hyd->LinkFlows[i]) > TINY)
            {
                ...
                f = 39.725 * h * pow(net->Link[i].Diam, 5) /
                    net->Link[i].Len / SQR(hyd->LinkFlows[i]);
                x[i] = (REAL4)f;
            }
            else x[i] = 0.0f;
        }
