...
*/
{
  ...
  hyd->X_tmp = (double *)calloc(MAX((net->Nnodes + 1), (net->Nlinks + 1)),
                                sizeof(double));
  hyd->OldStat =
      (StatType *)calloc(net->Nlinks + net->Ntanks + 1, sizeof(StatType));
  ...
  switch (link->Type) {
    ...
    case EN_PUMP:
      ...
      /* Otherwise use inverse of power curve */
        ...
        x = ABS(h0 - dh) / (net->Pump[p].R * x), y = 1.0 / net->Pump[p].N;
        hyd->LinkFlows[k] = pow(x, y);
      }
      break;
    default:
...
void setlinkstatus(EN_Project *pr, int index, char value, StatType *s,
                   double *k)
/*----------------------------------------------------------------
  ...
  switch (link->Type) {
    ...
    case EN_PIPE:
      ...
      switch (hyd->Formflag) {
        ...
        case CM:
          link->R =
              SQR(4.0 * e / (1.49 * PI * d * d)) * pow((d / 4.0), -1.333) * L;
      }
      break;
...
*/
{
  ...
  /* remembering that volume curve is in original units.*/
    ...
    return (interp(curve->Npts, curve->X, curve->Y,
                   (h - net->Node[tank->Node].El) * pr->Ucf[HEAD]) /
            pr->Ucf[VOLUME]);
  }
...
*/
{
  ...
  /* Remember that volume curve is stored in original units.           */
    ...
    Scurve *curve = &net->Curve[j];
    return (net->Node[tank->Node].El +
            interp(curve->Npts, curve->Y, curve->X, v * pr->Ucf[VOLUME]) /
                pr->Ucf[HEAD]);
  }
...
*/
{
  ...
  *iter = 1;
  ...
  if (errcode < 0)
    errcode = 101; /* Memory allocation error */
  else if (errcode > 0) {
    ...
    if (n == n1 || n == n2) {
      ...
      if (t == EN_PRV || t == EN_PSV || t == EN_FCV) {
        if (hyd->LinkStatus[k] == ACTIVE) {
          if (rep->Statflag == FULL) {
            sprintf(pr->Msg, FMT61, clocktime(rep->Atime, time->Htime),
                    link->ID);
            writeline(pr, pr->Msg);
          }
          if (link->Type == EN_FCV) {
...
*/
{
  ...
  for (i = 1; i <= net->Nvalves; i++) /* Examine each valve   */
  {
    ...
    //      && ABS(hyd->LinkFlows[k]) < hyd->Qtol) continue;            /*
    //      negligible.          */
    ...
    switch (link->Type) /* Evaluate new status: */
    {
      case EN_PRV:
        ...
        hyd->LinkStatus[k] = prvstatus(pr, k, status, hset, hyd->NodeHead[n1],
                                       hyd->NodeHead[n2]);
        break;
      case EN_PSV:
        hset = net->Node[n1].El + hyd->LinkSetting[k];
        hyd->LinkStatus[k] = psvstatus(pr, k, status, hset, hyd->NodeHead[n1],
                                       hyd->NodeHead[n2]);
        break;

      ////  FCV status checks moved back into the linkstatus() function ////
      //         case FCV:  S[k] =
      //         fcvstatus(k,s,hyd->NodeHead[n1],hyd->NodeHead[n2]);
      //                    break;
...
*/
{
  ...
  for (k = 1; k <= net->Nlinks; k++) {
    ...
    if (link->Type == EN_CVPIPE) {
      hyd->LinkStatus[k] =
          cvstatus(pr, hyd->LinkStatus[k], dh, hyd->LinkFlows[k]);
    }
    ...
    if (link->Type == EN_FCV && hyd->LinkSetting[k] != MISSING) {  //
      hyd->LinkStatus[k] =
          fcvstatus(pr, k, status, hyd->NodeHead[n1], hyd->NodeHead[n2]);  //
    }
...
StatType prvstatus(EN_Project *pr, int k, StatType s, double hset, double h1,
                   double h2)
/*
**-----------------------------------------------------------
**  Input:   k    = link index
...
StatType psvstatus(EN_Project *pr, int k, StatType s, double hset, double h1,
                   double h2)
/*
**-----------------------------------------------------------
**  Input:   k    = link index
  ...
  for (i = 1; i <= net->Ncontrols; i++) {
    ...
    if (reset == 1) {
      ...
      if (link->Type >= EN_PRV) {
        if (hyd->LinkSetting[k] != net->Control[i].Setting) {
          change = 1;
        } else if (hyd->LinkSetting[k] == MISSING &&
                   s != net->Control[i].Status) {
          change = 1;
        }
      }
...
*/
{
  ...
  for (k = 1; k <= net->Nlinks; k++) {
    ...
    switch (link->Type) {
      ...
      case EN_FCV:
      case EN_PRV:
      case EN_PSV: /* If valve status fixed then treat as pipe, otherwise ignore
                      the valve for now. */
        if (hyd->LinkSetting[k] == MISSING) {
...
*/
{
  double q, /* Abs. value of flow */
      f;    /* Friction factor    */
  double x1, x2, x3, x4, y1, y2, y3, fa, fb, r;
  double s, w;
  ...
  *dfdq = 0.0;
  if (link->Type > EN_PIPE) return (1.0); /* Only apply to pipes */
  q = ABS(hyd->LinkFlows[k]);
...
*/
{
  ...
  if (hyd->LinkSetting[k] != MISSING) {
    link->Km =
        0.02517 * hyd->LinkSetting[k] / (SQR(link->Diam) * SQR(link->Diam));
  }
